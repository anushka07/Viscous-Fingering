...
for istep= 1:NSTEP;
  if (istep==1)
   ...
   % J1 = J(ux,uy,c,csx,kx,ky);
    ...
    %Update velocities
    [Psi,ux,uy]= streamf(ux,uy,c0,cs,csx,R,De,n,kx,ky,k2i);
    ...
    %Predicted variables
   for j=1:3
    [Psi,ux,uy]= streamf(ux,uy,cp,cs,csx,R,De,n,kx,ky,k2i);
   ...
   %dt= 1/(8*N*max(max(ux)));
    ...
    if (mod(istep,20)==0) | (istep==1);
        ...
        view([0,0,1]);axis tight;axis equal; title(['t=  ' num2str(tc)]);colormap(jet);drawnow
...
function [Psi,ux,uy]= streamf(ux,uy,c,cs,csx,R,De,n,kx,ky,k2i)
...
N = -uy.*(csx + dcx)+ux.*(dcy)+dcy;
O = (c+cs).*(((n-1)*De)./(1+De^2.*v2)).*(((ux+1).*(ux+1)).*duxy - uy.*uy.*duyx - 2*(ux+1).*uy.*duxx);
P = ((n-1)/2)*log(1+De^2.*v2);
M = P.*N;
